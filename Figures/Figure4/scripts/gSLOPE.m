...
meanLGTH      = zeros(1,length(K));
meanLGTH_t    = zeros(1,length(K));
...
lambda2 = LambdaGENERAL2(n, q1, Lgths, W);
...
for jj = 1:length(K)
    ...
    parfor ii = 1: iter
        ...
        % getting solutions
        [~, SL_01]   = iGroupSLOPE(X, y, lambda1, I, W, options);
        SL_01(abs(SL_01)<eps)   = 0;
        meanLGTHs   = [meanLGTHs, Lgths(SL_01>0)];
        TDidx       = intersect(find(SL_01>0), group_supp);
        meanLGTHs_t = [meanLGTHs_t, Lgths(TDidx)];
    ...
    meanLGTH_t(jj)  = mean(meanLGTHs_t);
end

 %% saving
cd('path to the directory where results should be saved') 
 
 save FDR_01n.txt -ascii FDR_01;
 save stdFDR_01n.txt -ascii stdFDR_01;
 save POWER_01n.txt -ascii POWER_01;
 save stdPOWER_01n.txt -ascii stdPOWER_01;
 save meanLGTH_gSLOPEn.txt -ascii meanLGTH;
 save meanLGTH_t_gSLOPEn.txt -ascii meanLGTH_t;

...
path  <- "path to \\Figures\\Figure6\\data\\";
setwd(path)
...
p <- p + theme( legend.position = c(.05, .95), legend.justification = c("left", "top"), legend.box.background = element_rect())
...
p <- p + geom_boxplot(aes(fill=labels), outlier.shape=15, position = position_dodge(width=0.7), width = 0.6, lwd=1.1, col = c("black") ) # aes(fill=labels),
p <- p + scale_x_discrete(breaks = c("A", "B", "C"), labels = c("AA", "Aa", "aa"))
p <- p + coord_cartesian(ylim = c(-1.05, 2.8) )
...
<!-- plots could be found in \\Figures6_files\\figure-html\\ directory -->
